import m3 from "../assets/hero/m3.jpg";
import m1 from "../assets/hero/m1.jpg";
import iman from "../assets/hero/iman.jpg";
import man from "../assets/hero/man.jpg";
import m6 from "../assets/hero/m6.jpg";
import i1   from "../assets/hero/i1.jpg";
import { useEffect } from "react";
import { Link } from "react-router-dom";

export default function Manufacturing() {
  useEffect(() => {
    window.scrollTo(0, 0);
  }, []);

  return (
    <>
      {/* Hero Section */}
      <section className="bg-sky-50 py-20">

  <div className="max-w-7xl mx-auto px-6">

    {/* Label */}
    <span className="inline-block px-4 py-1.5 rounded-full text-sm border border-slate-200 text-slate-600 mb-4">
      Trusted by 150+ manufacturing plants
    </span>

    {/* Heading */}
    <h1 className="text-4xl md:text-5xl xl:text-6xl font-bold leading-tight text-slate-900 mb-12">
      {" "}
      <span className="bg-gradient-to-r from-sky-500 to-sky-500 bg-clip-text text-transparent">
        Manufacturing
      </span>
    </h1>

    {/* Image */}
    <div className="relative rounded-3xl overflow-hidden shadow-2xl border border-slate-100">

      <img
        src={iman}
        alt="Digital dashboards on factory floor"
        className="w-full h-[520px] object-cover"
      />

      {/* CTA Button */}
      <div className="absolute bottom-8 right-8">
        <Link
          to="/contact"
          className="group inline-flex items-center gap-2 px-7 py-3 rounded-xl bg-white text-black font-medium hover:bg-slate-200 transition shadow-lg"
        >
          Business Enquiry
          <span className="transform transition group-hover:translate-x-1">→</span>
        </Link>
      </div>

    </div>

    {/* Stats */}
    <div className="grid grid-cols-3 gap-8 pt-12 max-w-lg">
      <div>
        <p className="text-3xl font-bold text-slate-900">25%</p>
        <p className="text-sm text-slate-500 mt-1">Higher Productivity</p>
      </div>

      <div>
        <p className="text-3xl font-bold text-slate-900">40%</p>
        <p className="text-sm text-slate-500 mt-1">Faster Issue Resolution</p>
      </div>

      <div>
        <p className="text-3xl font-bold text-slate-900">99.9%</p>
        <p className="text-sm text-slate-500 mt-1">System Reliability</p>
      </div>
    </div>

  </div>

</section>

      {/* Real-World Impact Section */}
      <section className="max-w-7xl mx-auto px-6 py-24 grid md:grid-cols-2 gap-16 items-center">
        
        {/* Left Image */}
        <div className="relative">
          <div className="rounded-3xl overflow-hidden shadow-xl">
            <img
              src={m1}
              alt="Workers viewing production dashboard in factory"
              className="w-full h-[480px] object-cover"
            />
          </div>
        </div>

        {/* Right Content */}
        <div>
          <p className="text-sm font-semibold tracking-widest text-sky-600 uppercase mb-4">
            Real-World Impact
          </p>

          <h2 className="text-3xl md:text-4xl font-bold text-slate-900 leading-tight">
            Make Your Factory Floor Smarter
          </h2>

          <p className="mt-6 text-slate-600 max-w-xl leading-relaxed">
            Real-time production dashboards and safety signage ensure that operators, 
            supervisors, and managers stay aligned and act faster on issues.
          </p>

          <ul className="mt-8 space-y-4">
            <li className="flex items-start gap-3">
              <span className="flex items-center justify-center w-6 h-6 rounded-full bg-slate-900 text-white text-sm">✓</span>
              <span className="text-slate-700">Live production KPIs & OEE dashboards</span>
            </li>
            <li className="flex items-start gap-3">
              <span className="flex items-center justify-center w-6 h-6 rounded-full bg-slate-900 text-white text-sm">✓</span>
              <span className="text-slate-700">Safety alerts & compliance messages</span>
            </li>
            <li className="flex items-start gap-3">
              <span className="flex items-center justify-center w-6 h-6 rounded-full bg-slate-900 text-white text-sm">✓</span>
              <span className="text-slate-700">Maintenance & downtime notifications</span>
            </li>
            <li className="flex items-start gap-3">
              <span className="flex items-center justify-center w-6 h-6 rounded-full bg-slate-900 text-white text-sm">✓</span>
              <span className="text-slate-700">Shift performance visibility</span>
            </li>
          </ul>
        </div>
      </section>

      {/* Use Cases Section */}
      <section className="bg-white py-28">
        <div className="max-w-7xl mx-auto px-6">

          <div className="text-center max-w-3xl mx-auto mb-16">
            <p className="text-sm tracking-widest text-slate-500 uppercase">
              Use Cases
            </p>
            <h2 className="text-3xl md:text-4xl font-bold text-slate-900 mt-3">
              Built for Smart Manufacturing
            </h2>
            <p className="mt-4 text-slate-600">
              Digital display solutions for operational excellence.
            </p>
          </div>

          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">

            <div className="group rounded-3xl border border-slate-200 bg-white overflow-hidden shadow-sm hover:shadow-xl transition">
              <div className="h-48 overflow-hidden">
                <img src={man} alt="Production Dashboards" className="w-full h-full object-cover group-hover:scale-105 transition duration-500" />
              </div>
              <div className="p-8">
                <h3 className="text-xl font-semibold text-slate-900 mb-3">
                  Production Dashboards
                </h3>
                <p className="text-slate-600">
                  Real-time KPIs, throughput, and machine performance on large screens.
                </p>
              </div>
            </div>

            <div className="group rounded-3xl border border-slate-200 bg-white overflow-hidden shadow-sm hover:shadow-xl transition">
              <div className="h-48 overflow-hidden">
                <img src={m6} alt="Safety Signage" className="w-full h-full object-cover group-hover:scale-105 transition duration-500" />
              </div>
              <div className="p-8">
                <h3 className="text-xl font-semibold text-slate-900 mb-3">
                  Safety Signage
                </h3>
                <p className="text-slate-600">
                  Broadcast safety instructions, compliance reminders, and emergency alerts.
                </p>
              </div>
            </div>

            <div className="group rounded-3xl border border-slate-200 bg-white overflow-hidden shadow-sm hover:shadow-xl transition">
              <div className="h-48 overflow-hidden">
                <img src={i1} alt="Maintenance Alerts" className="w-full h-full object-cover group-hover:scale-105 transition duration-500" />
              </div>
              <div className="p-8">
                <h3 className="text-xl font-semibold text-slate-900 mb-3">
                  Maintenance Alerts
                </h3>
                <p className="text-slate-600">
                  Display scheduled maintenance, downtime alerts, and repair priorities.
                </p>
              </div>
            </div>

          </div>
        </div>
      </section>

      {/* Business Benefits Section */}
      <section className="relative bg-[#0b0b0e] py-28">
        <div className="absolute inset-0 bg-gradient-to-b from-black/60 to-black/90" />

        <div className="relative max-w-7xl mx-auto px-6 text-center">
          <p className="text-sm tracking-widest text-slate-400 uppercase">
            Business Benefits
          </p>
          <h2 className="mt-4 text-3xl md:text-4xl lg:text-5xl font-bold text-white">
            Manufacturing That Runs on Real-Time Data
          </h2>
          <p className="mt-5 text-slate-400 max-w-3xl mx-auto">
            Improve productivity, safety, and operational visibility with digital factory solutions.
          </p>

          <div className="mt-20 grid sm:grid-cols-2 lg:grid-cols-4 gap-12">

            <div className="flex flex-col items-center text-center">
              <p className="text-4xl font-bold text-white">25%</p>
              <h4 className="mt-2 font-semibold text-white">Higher Productivity</h4>
              <p className="mt-3 text-slate-400 text-sm">
                Operators act faster with real-time insights.
              </p>
            </div>

            <div className="flex flex-col items-center text-center">
              <p className="text-4xl font-bold text-white">40%</p>
              <h4 className="mt-2 font-semibold text-white">Faster Issue Resolution</h4>
              <p className="mt-3 text-slate-400 text-sm">
                Live alerts reduce downtime and bottlenecks.
              </p>
            </div>

            <div className="flex flex-col items-center text-center">
              <p className="text-4xl font-bold text-white">30%</p>
              <h4 className="mt-2 font-semibold text-white">Improved Safety Compliance</h4>
              <p className="mt-3 text-slate-400 text-sm">
                Safety messages reduce incidents on the floor.
              </p>
            </div>

            <div className="flex flex-col items-center text-center">
              <p className="text-4xl font-bold text-white">99.9%</p>
              <h4 className="mt-2 font-semibold text-white">System Reliability</h4>
              <p className="mt-3 text-slate-400 text-sm">
                Always-on displays for critical operations.
              </p>
            </div>

          </div>
        </div>
      </section>
    </>
  );
}